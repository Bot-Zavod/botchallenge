...
class Mixin:
    ...
    def get_actionspace(self) -> Set[Tuple[int, int]]:
        ...
        hero = self._hero
        hero_step = {(hero[0] + act[0], hero[1] + act[1]) for act in directions}
        hero_jump = {(hero[0] + act[0]*2, hero[1] + act[1]*2) for act in directions}
        
        first_lasers = set(laser for lasers in self.first_lasers for laser in lasers)  # lasers position on first step
        second_lasers = self.check_second_lasers()  # lasers position on second step
        ...
        for i, lasers in enumerate(first_lasers):
            for laser in lasers:
                if laser not in second_layer_barrier:
                    second_lasers.add(
                        (laser[0] + directions[i][0], laser[1] + directions[i][1])
                    )
